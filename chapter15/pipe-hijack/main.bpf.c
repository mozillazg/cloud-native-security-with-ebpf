...
//
//    struct *pipe_pair_t pipes[MAX_PIPE_NUM] = {};
//    struct *pipe_pair_t old_pipes[MAX_PIPE_NUM] = bpf_map_lookup_elem(&pid_pipes_map, &pid);
//    if (old_pipes) {
//
//    }
//    bpf_map_update_elem(&pid_pipes_map, &pid, &pipes, BPF_ANY);
...
//    if (fd2 == 0 && !str_eq(comm, "bash", 16)) {
//        return 0;
//    }
//    if (fd2 == 1 && !str_eq(comm, "curl", 16)) {
//        return 0;
//    }
    if (fd1 > 1 && fd2 > 1) {
        return 0;
    ...
    u8 zero = 0;
    bpf_map_update_elem(&dup_events_map, &pid, &zero, BPF_ANY);
...
//    }
    ...
    if (!bpf_map_lookup_elem(&dup_events_map, &pid)) {
        return 0;
    }
